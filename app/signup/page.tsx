...
export default function SignupPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    const { data, error } = await supabase.auth.signUp({ email, password, options: { data: { username: formData.username } } })
    setLoading(false)
    if (error) {
      setError(error.message)
    } else if (data.user) {
      // User is signed up and session is created (if email confirmation is off)
          ...
          <div className="lg:w-1/2 flex flex-col justify-center p-6 sm:p-8">
            <div className="w-full">
              ...
              <div className="text-center mb-4">
                ...
                <p className="text-sm text-gray-600">Join thousands on their wellness journey</p>
              </div>
              ...
              <form onSubmit={handleSubmit} className="space-y-3">
                ...
                <button
                  ...
                  className="w-full bg-gradient-to-r from-purple-600 to-indigo-600 text-white py-2 px-4 rounded-lg hover:from-purple-700 hover:to-indigo-700 transition-all duration-300 font-medium shadow-md shadow-purple-500/25 hover:shadow-purple-500/40 disabled:opacity-50 disabled:cursor-not-allowed"
                >
